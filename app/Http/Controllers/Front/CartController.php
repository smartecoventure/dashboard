...
class CartController extends Controller
{
    ...
    public function cart()
    {
        ...
        foreach($products as $product){
            $shipping_fee += $product['item']['ship_fee'];
        }
        
        $totalPrice = $cart->totalPrice;
        $mainTotal = $totalPrice + $shipping_fee;
        ...
        return view('front.cart', compact('products','totalPrice','shipping_fee','mainTotal','tx')); 
    }
   ...
   public function addtocart($id)
    {
        $prod = Product::where('id','=',$id)->first(['id','user_id','slug','name','photo','size','ship_fee','size_qty','size_price','color','price','stock','type','file','link','license','license_qty','measure','whole_sell_qty','whole_sell_discount','attributes']);
        ...
        // Set Ship Fee
        $ship_fee = '';
        if(!empty($prod->ship_fee)){ 
            $ship_fee = trim($prod->ship_fee[0]);
        }  
        ...
        $cart->add($prod, $prod->id, $size, $ship_fee, $color, $keys, $values);
   ...
   public function addcart($id)
    {
        $prod = Product::where('id','=',$id)->first(['id','user_id','slug','name','photo','size','size_qty','ship_fee','size_price','color','price','stock','type','file','link','license','license_qty','measure','whole_sell_qty','whole_sell_discount','attributes']);
    ...
    public function addnumcart()
    {
        ...
        $oldCart = Session::has('cart') ? Session::get('cart') : null;
        $cart = new Cart($oldCart);
        $cart->addnum($prod, $prod->id, $qty, $size, $ship_fee, $color,$size_qty,$size_price,$size_key,$keys,$values);
        if($cart->items[$id.$size.$ship_fee.$color.str_replace(str_split(' ,'),'',$values)]['dp'] == 1)
        {
            return 'digital';
        }
        if($cart->items[$id.$size.$ship_fee.$color.str_replace(str_split(' ,'),'',$values)]['stock'] < 0)
        {
            return 0;
        }

        if($cart->items[$id.$size.$ship_fee.$color.str_replace(str_split(' ,'),'',$values)]['size_qty'])
        {
            if($cart->items[$id.$size.$ship_fee.$color.str_replace(str_split(' ,'),'',$values)]['qty'] > $cart->items[$id.$size.$color.str_replace(str_split(' ,'),'',$values)]['size_qty'])
            {
                return 0;
            }           
        }
    ...
    public function addtonumcart()
    {
        ...
        $oldCart = Session::has('cart') ? Session::get('cart') : null;
        $cart = new Cart($oldCart);
        $cart->addnum($prod, $prod->id, $qty, $ship_fee, $size,$color,$size_qty,$size_price,$size_key,$keys,$values);
        if($cart->items[$id.$size.$ship_fee.$color.str_replace(str_split(' ,'),'',$values)]['dp'] == 1)
        {
            return redirect()->route('front.cart')->with('unsuccess',$lang->already_cart);  
        }
    ...
    public function addbyone()
    {
        ...
        $oldCart = Session::has('cart') ? Session::get('cart') : null;
        $cart = new Cart($oldCart);
        $cart->adding($prod, $itemid, $size_qty, $size_price, $shipping_fee);
        ...
        if (count($cart->items) > 0) {
            ...
            Session::put('cart', $cart);
                ...
                if($gs->currency_format == 0){
                    ...
                    $data[5] = $curr->sign.round($data[5] * $curr->value,2);
            
                }
                else{
                    $data[0] = round($data[0] * $curr->value,2).$curr->sign;
                    $data[3] = round($data[3] * $curr->value,2).$curr->sign;
                    $data[5] = round($data[5] * $curr->value,2).$curr->sign;
                }

...
class CheckoutController extends Controller
{
    ...
    public function checkout()
    {
        ...
        if(Auth::guard('web')->check()){
                ...
                $total = $cart->totalPrice + $shipping_fee;
            ...
            return view('front.checkout', ['products' => $cart->items, 'total_price_products'=>$cart->totalPrice, 'shipping_fee' => $shipping_fee, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'digital' => $dp, 'curr' => $curr,'shipping_data' => $shipping_data,'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);             
        }else{
            // If guest checkout is activated then user can go for checkout
           	if($gs->guest_checkout == 1){
                ...
                $total_price_products = $cart->totalPrice;
                ...
                foreach ($products as $prod) {
                    if($prod['item']['type'] != 'Physical')
                    {
                        ...
                        {
                $ck = 1;
                return view('front.checkout', ['products' => $cart->items, 'total_price_products'=>$total_price_products, 'shipping_fee' => $shipping_fee, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'checked' => $ck, 'digital' => $dp, 'curr' => $curr,'shipping_data' => $shipping_data,'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);  
                        }
                    }
                }
            return view('front.checkout', ['products' => $cart->items, 'total_price_products'=>$total_price_products, 'shipping_fee' => $shipping_fee, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'digital' => $dp, 'curr' => $curr,'shipping_data' => $shipping_data,'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);                 
